...
object IPApp_lab4 {
    ...
    def generateDecisionTreeModel(sc: SparkContext): Unit = {
      if (Files.exists(Paths.get(IPSettings_lab4.DECISION_TREE_PATH))) {
        println(s"${IPSettings_lab4.DECISION_TREE_PATH} exists, skipping Random Forest model formation..")
        return
      }
     ...
     // val numOfTrees = 4 to(10, 1)
      ...
      val randomDecisionTreeModel = DecisionTree.trainClassifier(parsedData, numClasses, categoricalFeaturesInfo, bestimpurity, bestmaxdepth, maxBins)


      // Save and load model
      randomDecisionTreeModel.save(sc, IPSettings_lab4.DECISION_TREE_PATH)
    ...
    def classifyImage(sc: SparkContext, path: String): Double = {
      ...
      val nbModel = DecisionTreeModel.load(sc, IPSettings_lab4.DECISION_TREE_PATH)
