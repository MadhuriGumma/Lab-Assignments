/**
  * Created by sudhakar on 2/23/17.
  */
...
//val response = "Test data for connection"
  ...
  val bytes = new ByteArrayInputStream(imageByte)
   ...
   val response = IPApp.testImage("image.png")
    ...
    httpExchange.getResponseHeaders().add("Access-Control-Allow-Origin", "http://localhost:8081")
